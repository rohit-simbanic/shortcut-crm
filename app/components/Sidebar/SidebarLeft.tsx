...
const SidebarLeft = () => {
  const handleMouseEnter = () => {
    setCollapse(false); // Expand the sidebar when hovering
  };

  const handleMouseLeave = () => {
    setCollapse(true); // Collapse the sidebar when not hovering
  };
    ...
    <div className="border-r border-gray-300 p-5 relative z-40 bg-[#f8f9fb] dark:bg-black min-h-screen">
      ...
      >
        ...
        <nav>
          ...
          <div className="flex flex-col gap-[4px]">
            ...
            {isDropdownOpen && (
              <div
                id="dropdown-menu"
                className="mt-2 ml-[9px] border-l-2 dark:border-slate-50 list-text"
              >
                <div
                  className="py-0 p-1"
                  role="menu"
        ...
        <ul className="flex flex-col gap-[3px] mt-8">
          <li className="flex gap-2 hover:bg-[#e4e8eb] hover:cursor-pointer px-1 py-1 rounded-md items-center leading-3">
            <svg
              width="22"
              height="22"
              viewBox="0 0 20 20"
